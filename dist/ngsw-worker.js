(function() {
  ...
  class AssetGroup {
    constructor(scope, adapter, idle, config, hashes, db, prefix) {
      ...
      this.origin = this.adapter.parseUrl(this.scope.registration.scope, this.scope.registration.scope).origin;
    }
    cacheStatus(url) {
          ...
          if (cachedResponse !== undefined) {
            ...
            if (this.hashes.has(url)) {
              ...
              if (yield this.needToRevalidate(req, cachedResponse)) {
                this.idle.schedule(`revalidate(${this.prefix}, ${this.config.name}): ${req.url}`, () =>
    ...
    fetchAndCacheOnce(req, used = true) {
      return __awaiter(this, void 0, void 0, function*() {
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error(
              `Response not Ok (fetchAndCacheOnce): request for ${req.url} returned response ${res.status} ${
                res.statusText
              }`
            );
          }
        ...
        if (this.hashes.has(url)) {
          ...
          if (makeCacheBustedRequest) {
            ...
            if (!cacheBustedResult.ok) {
              throw new SwCriticalError(
                `Response not Ok (cacheBustedFetchFromNetwork): cache busted request for ${req.url} returned response ${
                  cacheBustedResult.status
            ...
            if (canonicalHash !== cacheBustedHash) {
              throw new SwCriticalError(
                `Hash mismatch (cacheBustedFetchFromNetwork): ${
                  req.url
                }: expected ${canonicalHash}, got ${cacheBustedHash} (after cache busting)`
              );
            }
      ...
      return __awaiter(this, void 0, void 0, function*() {
        ...
        if (updateFrom !== undefined) {
          ...
          yield (yield updateFrom.previouslyCachedResources())
            ...
            .filter(
              url =>
                this.config.urls.some(cacheUrl => cacheUrl === url) || this.patterns.some(pattern => pattern.test(url))
            )
  ...
  class AppVersion {
    constructor(scope, adapter, database, idle, manifest, manifestHash) {
      ...
      this.dataGroups = (manifest.dataGroups || []).map(
        config => new DataGroup(this.scope, this.adapter, config, this.database, `ngsw:${config.version}:data`)
      );
  ...
  class Driver {
    ...
    initialize() {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        try {
          ...
          this.idle.schedule('init post-load (update, cleanup)', () =>
            __awaiter$5(this, void 0, void 0, function*() {
              yield this.checkForUpdate();
              try {
      ...
      return __awaiter$5(this, void 0, void 0, function*() {
        yield (yield this.scope.caches.keys())
          ...
          .reduce(
            (previous, key) =>
              __awaiter$5(this, void 0, void 0, function*() {
                yield Promise.all([previous, this.scope.caches.delete(key)]);
              }),
            Promise.resolve()
          );
      });
    ...
    versionFailed(appVersion, err, latest) {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        if (this.latestHash === brokenHash || latest) {
          ...
          Array.from(this.clientVersionMap.keys()).forEach(clientId => this.clientVersionMap.delete(clientId));
        } else {
          ...
          const affectedClients = Array.from(this.clientVersionMap.keys()).filter(
            clientId => this.clientVersionMap.get(clientId) === brokenHash
          );
    ...
    cleanupCaches() {
      return __awaiter$5(this, void 0, void 0, function*() {
        ...
        knownClients.filter(id => activeClients.indexOf(id) === -1).forEach(id => this.clientVersionMap.delete(id));
